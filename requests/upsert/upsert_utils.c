...
char* get_schema_file_content(sdb_http_response_t* http_response, char* schema_path,
  ...
  s_compile(
      ...
      "/schema?col=%s&id=%s with an example of your valid JSON schema and "
      "try again.",
      col_path, queries.col, queries.id);
  ...
  s_compile(
      ...
      "requested document: %s - use POST /schema?col=%s&id=%s with an example "
